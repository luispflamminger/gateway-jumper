...
//	      claims.put("env", envName);
	...
	public TokenInfo getAccessToken(String token_endpoint2, String tif_clientID2, String tif_clientSecret2, boolean autoevent) {
		...
		if (gwAccessToken == null) {
			...
			if (autoevent) {
				try {
					gwAccessToken = getTokenInfoMono(token_endpoint2, cc).toFuture().thenApplyAsync(tokenInfo -> tokenInfo).get(15, TimeUnit.SECONDS);
...
;
		return responseMono.flatMap(response -> {
			...
			log.debug("Got token: {}", accessToken);
